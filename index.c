/*
    ...
    Alguns padrões foram seguidos a partir do "index-exemplo" fornecido para o auxílio da execução do trabalho. Por isso, partes do código estão em inglês.
*/
...
// Recebe o fp apontando para a pagina a ser lida
btPage *lePaginaDoArquivo(FILE *fp);
// Escreve a pagina num arquivo numa posição especifica do rrn
...
// Cria uma nova pagina ou arvore, inicializa seus valores apos alocar memoria
...
unsigned short int insercaoArvore(record *newRecord, btPage **root, FILE *fp) {
    ...
    promotedKey *kingKey = (promotedKey *) malloc(sizeof(promotedKey));
...
// Retorna rrn se a chave existe, se não retorna -1
// Essa funcao depende da pegaPagina e da pegaOuCriaRaiz
...
btPage *lePaginaDoArquivo(FILE *fp) {
    ...
    page -> childs = (long *) malloc((MAXKEYS + 1)*sizeof(long));
...
void escreve_pagina(btPage *page, FILE *fp)
{
    ...
    char *fill_empty_space = (char *) malloc(FREE_SPACE_ON_PAGE*sizeof(char));
...
// Recebe o fp em qualquer posicao.
btPage *pegaPagina(long rrn, FILE *fp) {
...
void escreveCabecalhoArvore(FILE *fp, long rootRRN) {
    ...
    char *fill_empty_space = (char *) malloc(TREE_HEADER*sizeof(char));
...
btPage *criaArvore(FILE *fp) {
    btPage *root = (btPage *) malloc(sizeof(btPage));

    root -> records = (record *) malloc(MAXKEYS*sizeof(record));
    root -> childs = (long *) malloc((MAXKEYS + 1)*sizeof(long));

    root -> numberOfKeys = 0;
    root -> isLeaf = 1;

    // setando todos os valores inicializados para -1, visto que o lixo
    // de memória oriundo do malloc pode ser um rrn valido ou o rrn de uma pagina filha
    for (int i = 0; i < MAXKEYS; i++)
    {
...
void divide_e_ajusta(btPage *page_in_left, btPage *page_in_right) {
    ...
    // Em uma pagina cheia, todos os filhos possuem ponteiros (segundo a aula de insercao)
